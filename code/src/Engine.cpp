...
void Engine::initEngine(const char* title, int xPos, int yPos, int width,
  ...
  if (IMG_Init(IMG_INIT_JPG | IMG_INIT_PNG | IMG_INIT_TIF) == 0) {
    SDL_Log("[ERR] Unable to initialize IMG: %s", SDL_GetError());
    exit(EXIT_FAILURE);
  }
  ...
  if (Mix_Init(MIX_INIT_FLAC | MIX_INIT_OGG | MIX_INIT_MID | MIX_INIT_MP3 |
    ...
    SDL_Log("[ERR] Unable to initialize MIX: %s", SDL_GetError());
    exit(EXIT_FAILURE);
  }
  ...
  if (this->window == nullptr) {
    SDL_Log("[ERR] Window is null: %s", SDL_GetError());
    exit(EXIT_FAILURE);
  }
